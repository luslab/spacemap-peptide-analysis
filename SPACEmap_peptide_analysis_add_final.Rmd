...
uniprotid.db.total.num.collated.db = clf.ipr.matches.overlap.cbp_rbp.collated %>% 
  filter(is.cbp) %>%
  pull(Uniprot.ID) %>%
  unique() %>%
  length()

clf.ipr.matches.overlap.cbp_rbp.collated.db_freq = clf.ipr.matches.overlap.cbp_rbp.collated %>%
  filter(is.cbp) %>%
...
write.table(clf.ipr.matches.overlap.cbp_rbp.collated.db_freq,
            "output/clf_db_protein_match_frequencies.tsv",
            quote = F,
            sep = "\t",
            row.names = F)

clf.ipr.matches.overlap.cbp_rbp.collated.db_freq %>%
  head(10) %>%
  mutate(ipr.description = factor(ipr.description, levels = ipr.description)) %>%
