...
```{r}
clf.ipr.matches %>%
  filter(uniprot.id == "A2A4P0")
```

Plot an example of scanning results for a particular protein:
...
calc_cumulative_row_number = function(current.ipr.n, protein.df) {
  return(protein.df %>%
           filter(ipr.n <= current.ipr.n) %>%
           dplyr::select(ipr.n,
                         ipr.row.number) %>%
           distinct() %>%
           pull(ipr.row.number) %>%
...
preprocess_protein_df = function(protein.df) {
  c = 5 # vertical space between InterPro entries
  h = 5 # height of a match rectangle
  d = 2 # vertical space between rectangles
...
draw_matches = function(uniprot.id) {
  ...
  ggplot(protein.df) +
    ...
    geom_text(data = ipr.df,
              ...
              size = label.font.size) +
    # Ruler
    xlim(1, unlist(protein.df[1, "length"]) + x.axis.extension) +
    theme_classic() 
}
```

```{r}
# protein.df = generate_protein_df("A2A4P0")
# preprocess_protein_df(protein.df)
draw_matches("A2A4P0")
```
