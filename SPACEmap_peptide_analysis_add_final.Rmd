...
Count CLF peptides (from DNA- and RNA-binding proteins) that match domains, IDRs, and those who do match any feature:
...
clf.peptides.matches = clf.peptides.coord %>%
  ...
  group_by(Uniprot.ID, 
           ...
           N.term.cleavage.window, 
           C.term.cleavage.window) %>%
  do(mutate(., matched = ifelse("Domain" %in% .$matched.feature,
                                "Domain",
...
So, CLF peptides from DNA- and RNA-binding proteins stratify by their matches as follows:

```{r, include=T}
peptides.matched.domain.n = clf.peptides.matches %>% 
  filter(matched == "Domain") %>%
  dplyr::select(Uniprot.ID, 
                Sequence, 
                N.term.cleavage.window, 
                C.term.cleavage.window) %>%
...
peptides.matched.idr.n = clf.peptides.matches %>% 
  filter(matched == "IDR") %>%
  dplyr::select(Uniprot.ID, 
                Sequence, 
                N.term.cleavage.window, 
...
peptides.matched.none.n = clf.peptides.matches %>% 
  filter(matched == "None") %>%
  dplyr::select(Uniprot.ID, 
                Sequence, 
                N.term.cleavage.window, 
...
peptides.matched.all.n = clf.peptides.matches %>% 
  dplyr::select(Uniprot.ID, 
                Sequence, 
                N.term.cleavage.window, 
                C.term.cleavage.window) %>%
...
cat("Domain:", peptides.matched.domain.n, paste0("(", round(peptides.matched.domain.n / peptides.matched.all.n * 100, 2), "%)\n"))
cat("IDR   :", peptides.matched.idr.n,    paste0("(", round(peptides.matched.idr.n    / peptides.matched.all.n * 100, 2), "%)\n"))
cat("None  :", peptides.matched.none.n,   paste0("(", round(peptides.matched.none.n   / peptides.matched.all.n * 100, 2), "%)\n"))
```

## Annotate InterPro entries matched by CLF peptides
...
Check the proportion of putative DBDs among all matched InterPro entries:

```{r, include=T}
ipr.draft.classes.clf.putative_dbd.n = ipr.draft.classes.clf %>%
  filter(category == "Putative_DBD") %>%
  pull(ipr.accession) %>%
  unique() %>%
  length()
...
cat("Number of putative DBDs:", 
    ipr.draft.classes.clf.putative_dbd.n, 
    paste0("(", round(ipr.draft.classes.clf.putative_dbd.n / ipr.draft.classes.clf.all.n * 100, 2), "%)\n"))
```

Check the proportion of peptides matched by putative DBDs among all peptides:

```{r, include=T}
ipr.draft.classes.clf.putative_dbd.iprs = ipr.draft.classes.clf %>%
  filter(category == "Putative_DBD") %>%
  pull(ipr.accession) %>%
  unique()

clf.peptides.matches.putative_dbd_peptides.n = clf.peptides.matches %>%
  filter(match.peptide.overlap) %>%
  filter(ipr.accession %in% ipr.draft.classes.clf.putative_dbd.iprs) %>%
  dplyr::select(Uniprot.ID, 
                Sequence, 
                N.term.cleavage.window, 
                C.term.cleavage.window) %>%
...
clf.peptides.matches.putative_dbd_peptides.all = clf.peptides.matches %>%
  filter(match.peptide.overlap) %>%
  dplyr::select(Uniprot.ID, 
                Sequence, 
                N.term.cleavage.window, 
...
cat("Number of peptides matched by putative DBDs:", 
    clf.peptides.matches.putative_dbd_peptides.n, 
    paste0("(", round(clf.peptides.matches.putative_dbd_peptides.n / clf.peptides.matches.putative_dbd_peptides.all * 100, 2), "%)\n"))
```
