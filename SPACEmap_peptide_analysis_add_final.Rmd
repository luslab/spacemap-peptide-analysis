...
# clf.ipr.matches.base.prev = annot.table.prev %>%
#   left_join(clf.ipr.matches,
#             by = c("Uniprot.ID" = "uniprot.id")) %>%
#   left_join(peptide.table,
#             by = c("Uniprot.ID" = "Uniprot.ID")) %>%
#   filter(!is.na(ipr.accession))
# 
# clf.ipr.matches.overlap.prev = clf.ipr.matches.base.prev %>%
#   mutate(int.start = as.integer(int.start),
#          int.stop = as.integer(int.stop)) %>%
#   mutate(match.peptide.overlap = (if_else(int.stop < End.position,
#                                           int.stop,
...
#   filter(match.peptide.overlap)
# 
# # find the difference between the current and the previous set of matched IPRs
# iprs.prev = clf.ipr.matches.overlap.prev %>%
#   pull(ipr.accession) %>%
#   unique()
# 
# iprs.new = clf.ipr.matches.overlap %>%
#   pull(ipr.accession) %>%
#   unique()
# 
# # check that all such IPRs are matched by the peptides from the new SPACE proteins
# data.frame(ipr.accession = setdiff(iprs.new, iprs.prev)) %>%
#   left_join(clf.ipr.matches.overlap,
#             by = c("ipr.accession" = "ipr.accession"))
# 
# # but not from the old SPACE proteins
# data.frame(ipr.accession = setdiff(iprs.new, iprs.prev)) %>%
#   left_join(clf.ipr.matches.overlap.prev,
#             by = c("ipr.accession" = "ipr.accession"))
```
...
Find the number and proportion of all unmatched CLF peptides (from CBPs and RBPs) shared between SPACEmap and SPACE:
...
clf.all.peptides = sp.sp2.intersection.peptides
...
For the RNA-binding proteins (Fig. 4D; Fig. S4D, left):
...
We searched motifs _de novo_ in the sequences from the FASTA file using MeMe Suite v5.3.3 with default options, and, the significance level of e-value < 0.05, we found one motif (see output/meme_reports). It was inferred from just two proteins, nuclear receptor corepressors 1 (Ncor1, Q60974) and 2 (Ncor2, Q9WU42). The peptide that contains the motif immediately follows the silencing domain (IPR031557) in the N-terminal part of the proteins. This domain interacts with a corepressor G-protein pathway suppressor (GPS2) also known as SMRT. Hence, the novel motif could facilitate SMRT binding and, consequently, transcription repression.

<!-- ## Calculasp.sp2.intersection.peptideste peptide abundance in DBDs vs CLF/RF intensity ratio -->
...
Check CBPs where known DBDs are not matched by CLF peptides:
...
Most probably, these domains do not bind DNA. However, these non-DBD entries can still be relevant to chromatin, and proteins from which they come may have been bound to other chromatin proteins via these non-DBDs or IDRs, or may have used their IDRs to bind to DNA. 
...
Plot the final frequencies of CBPs with DBDs, that 

1) Have CLF peptides from DBDs.

2) Do not have CLF peptides from DBDs, but have CLF peptides from IDRs.

3) Do not have CLF peptides from IDRs, but have CLF peptides from non-DBD domains (expected, unclear, or unexpected).

4) Do not have CLF peptides from non-DBD domains, but have CLF peptides from non-domain or non-IDR regions.

```{r, include=T}
other.matches.classified = read.delim("output/space_spacemap-clf_shared_peptides_dbps_nondbds_final.tsv")
...
Hence, `r round((matched.proteins.n / all.proteins.n + idr.matched.proteins.n / all.proteins.n + other.domain.matched.expected.n / all.proteins.n) * 100, 1)`% of matches are either a DBD, or an IDR (both of which can be used to bind DNA), or another (but expected) domain (either a putative DBD or a protein-protein-interaction domain). A protein-protein-interaction domain could allow the proteins to bind to DNA indirectly, by contacting other chromatin proteins.

Check the total number of CBPs and the number of CBPs with known (classical) DBDs:

```{r, include=T}
cat("Total number of CBPs:", 
...
cat("Number of CBPs with classical DBDs:", 
    clf.ipr.matches.base %>% 
      filter(is.cbp) %>%
      filter(ipr.accession %in% dbd.table$ipr_accession) %>%
      pull(Uniprot.ID) %>%
