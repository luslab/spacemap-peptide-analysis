...
# input/Crosslinked_fraction_CLF_proteins.fa.tsv
clf.ipr.matches = read.delim("output/interproscan_matches_clf/SPACEmap_crosslinked_fraction_overlap_with_SPACE_protein.fa.tsv",
                             header = F,
...
generate_protein_df = function(UniProtId) {
  # input/Crosslinked_fraction_CLF_proteins.fa.tsv
  return(read.delim("output/interproscan_matches_clf/SPACEmap_crosslinked_fraction_overlap_with_SPACE_protein.fa.tsv",
                    header = F,
                    colClasses = c("character",
                                   "character",
                                   "numeric",
...
# input/Crosslinked_fraction_CLF.tsv
...
draw_peptides = function(UniProtId) {
  ...
  # input/Crosslinked_fraction_CLF_proteins.fa.tsv
  protein.length = read.delim("output/interproscan_matches_clf/SPACEmap_crosslinked_fraction_overlap_with_SPACE_protein.fa.tsv",
                              header = F,
                              colClasses = c("character",
                                             "character",
                                             "numeric",
...
Check that the proteins in the CLF peptide table are indeed shared between SPACEmap CLF and SPACE:
...
sp.sp2.intersection.proteins = sort(intersect(space.proteins, spacemap.proteins))
...
unique(sp.sp2.intersection.proteins == sp.sp2.intersection.proteins.from_table)
```
...
Calculate the number of peptides shared by SPACE and SPACEmap CLF:
...
Calculate the number and proportion of shared matched peptides for Fig. 4B (upper bar):
...
Write down the table of shared CLF peptides (Supplementary Table 2, spreadsheet "Peptides mapped to a region"):
...
Using InterProScan, we scanned all proteins containing at least one peptide from the SPACEmap released fraction, also captured by SPACE (see `scan_rf.sh`).

Load and process the results of scanning of proteins enriched in the released fraction of SPACEmap and also in SPACE:

```{r, include=T}
# Load the InterProScan results
# input/Released_fraction.txt_overlapped_with_SPACE.fa.tsv
rf.ipr.matches = read.delim("output/interproscan_matches_rf/SPACEmap_released_fraction_overlap_with_SPACE_protein.fa.tsv",
                             header = F,
                             colClasses = c("character",
                                            "character",
                                            "numeric",
